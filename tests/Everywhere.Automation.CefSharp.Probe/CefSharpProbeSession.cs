...
internal sealed class CefSharpProbeSession : IAsyncDisposable
{
    ...
    public CefSharpProbeSession(ProbeOptions options) => _options = options;
    ...
    public async Task<ProbeNavigationResult> NavigateAsync(string address, int? settleMilliseconds = null, CancellationToken cancellationToken = default)
    {
        ...
        var settleDelay = settleMilliseconds is { } milliseconds ? TimeSpan.FromMilliseconds(milliseconds) : _options.SettleDelay;
        ...
        try
        {
            ...
            {
                var started = await TestAppProcessController.StartAsync(CefSharpTestAppPath.Executable, "chat", 42, _options.NavigationTimeout, cancellationToken, "--url", uri.AbsoluteUri);
            ...
            else
            {
                ...
                timeoutSource.CancelAfter(_options.NavigationTimeout);
    ...
    public async Task<ProbeQueryResult> QueryAsync(string target, string directions, int offset, int? limit = null, int? targetTokenBudget = null, CancellationToken cancellationToken = default)
    {
        await _gate.WaitAsync(cancellationToken);
        try
        {
            ...
            var normalizedLimit = Math.Min(limit ?? _options.Limit, VisualQueryRequest.MaximumLimit);
            var normalizedTokenBudget = targetTokenBudget ?? _options.TargetTokenBudget;
            ...
            var stopwatch = Stopwatch.StartNew();
            using var turn = _context.BeginTurn();
            ...
            var content = prompt.ToString();
            var publishedTargetCount = turn.Count;
            turn.Complete();

            var status = _status ?? throw new InvalidOperationException("The CefSharp TestApp has no current status.");
            var queryIndex = ++_queryIndex;
            var host = GetSafeFileName(new Uri(status.Address ?? _options.Addresses[0].AbsoluteUri).Host);
            var outputPath = Path.Combine(_options.OutputDirectory, $"{queryIndex:000}-{host}.visual-context.txt");
            ...
            await AppendTranscriptAsync("query_visual", new { target, directions, offset, limit = normalizedLimit, targetTokenBudget = normalizedTokenBudget }, new { result.OutputPath, result.Elapsed, result.PublishedTargetCount, result.RetainedTargetCount, result.RetainedTurnCount, CharacterCount = content.Length }, null);
            return result;
        }
        catch (Exception exception)
        {
            await AppendTranscriptAsync("query_visual", new { target, directions, offset, limit, targetTokenBudget }, null, exception.Message);
            throw;
        }
        finally
        {
    ...
    public async Task<ProbeSessionStatus> GetStatusAsync(CancellationToken cancellationToken = default)
    {
        ...
        {
            ObjectDisposedException.ThrowIf(_isDisposed, this);
            var result = new ProbeSessionStatus(_controller is not null, _status?.Address, _status?.ProcessId, _status?.Roots.Select(static root => $"0x{root.NativeHandle:X}").ToArray() ?? [], _context.TargetCount, _context.RetainedTurnCount, _context.NextTargetId, _options.OutputDirectory);
    ...
    private async Task EnsureStartedAsync(CancellationToken cancellationToken)
    {
        ...
        var uri = _options.Addresses[0];
        var started = await TestAppProcessController.StartAsync(CefSharpTestAppPath.Executable, "chat", 42, _options.NavigationTimeout, cancellationToken, "--url", uri.AbsoluteUri);
        _controller = started.Controller;
        _status = started.ReadyStatus;
        await Task.Delay(_options.SettleDelay, cancellationToken);
    }
    ...
    private VisualTarget ResolveTarget(string target, VisualElementRetention retention)
    {
        if (string.Equals(target, "root", StringComparison.OrdinalIgnoreCase))
        {
            ...
            var root = _backend.Query(retention, VisualElementLocator.FromNativeWindow(rootHandle), VisualElementResolution.TopLevel) ?? throw new InvalidOperationException("The Windows reader did not resolve the CefSharp probe window.");
            return new ElementTarget { Element = root.Element };
        }

        if (!int.TryParse(target, NumberStyles.None, CultureInfo.InvariantCulture, out var id) || !_context.TryGetTarget(id, out var visualTarget)) throw new KeyNotFoundException($"Visual target '{target}' is unavailable. Use 'root' or a decimal ID returned by query_visual.");
        return visualTarget;
    }
    ...
    private async Task AppendTranscriptAsync(string operation, object? request, object? result, string? error)
    {
        ...
        await File.AppendAllTextAsync(Path.Combine(_options.OutputDirectory, "transcript.jsonl"), entry + Environment.NewLine);
    }
