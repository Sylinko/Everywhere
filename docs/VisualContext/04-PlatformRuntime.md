...
Screen capture returns physical pixels without a DPI property. The Avalonia adapter attaches normalized 96-DPI presentation metadata when it constructs a Bitmap. Display scale or coordinate-transform metadata, if later required, belongs in an explicit descriptor rather than the pixel buffer interface.
